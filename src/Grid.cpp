	...
	for (int gridCell = 0; gridCell < grid.size(); gridCell++)
	{
		for (int a = 0; a < speedOctopus[gridCell].size(); a++)
		{
			...
			for (; b < grid[gridCell].size(); b += 2)
			{
				...
				int particleInd2 = grid[gridCell][b + 1];
				vector<int>* nVec2 = neighbors[particleInd2];
				__m128 pPos2 = _mm_load_ps(posVec + 4 * particleInd2);
				...
				for (; c + 4 <= grid[neighbor_grid_index].size(); c += 4)
				{
					...
					dif1 = _mm_sub_ps(pPos2, oPos1);
					dif2 = _mm_sub_ps(pPos2, oPos2);
					dif3 = _mm_sub_ps(pPos2, oPos3);
					dif4 = _mm_sub_ps(pPos2, oPos4);

					dist1 = _mm_mul_ps(dif1, dif1);
					dist2 = _mm_mul_ps(dif2, dif2);
					dist3 = _mm_mul_ps(dif3, dif3);
					dist4 = _mm_mul_ps(dif4, dif4);

					dist = _mm_hadd_ps(_mm_hadd_ps(dist1, dist2), _mm_hadd_ps(dist3, dist4));
					...
					{
						if (vals[i] < CUTOFFVAL) {
							nVec2->push_back(grid[neighbor_grid_index][c + i]);
							//dVec->push_back(vals[i]);
						}
					}
				}
				for (; c < grid[neighbor_grid_index].size(); c++)
				{
					...
					/* END DISTANCE CALCULATION*/
					if (d < CUTOFFVAL) {
						nVec2->push_back(other_particle_index);
						//dVec->push_back(d);
					}
